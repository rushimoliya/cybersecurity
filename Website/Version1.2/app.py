...
# Encryption function
def encrypt_file(file, algorithm):
    key = Fernet.generate_key()
    fernet = Fernet(key)
    file_data = file.read()
    encrypted_data = fernet.encrypt(file_data)
    encrypted_file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'encrypted_' + file.filename)
    key_file_path = os.path.join(app.config['UPLOAD_FOLDER'], 'key_' + file.filename)
...
def encryption():
    if request.method == 'POST':
        ...
        encrypted_file_path, key_file_path = encrypt_file(file, algorithm)
...
@login_required
def download_ppt():
    return send_file('static/pdf/Educational_ppt.pdf', as_attachment=True)

@app.route('/download_guide')
@login_required
def download_guide():
    return send_file('static/pdf/Cyber Security_ A Comprehensive Guide.pdf', as_attachment=True)
